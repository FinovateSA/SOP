...
| **Download button** | `<a class="dl-btn" href="…">⬇ Label</a>` | small outlined pill for file downloads; usage guide has the CSS (`.dl-btn`) — copy it if your SOP needs one |
| **Expander** | `<details class="tk"><summary>Label</summary><div class="inner">…</div></details>` | collapsible section for long content (prompts, code) so a card stays scannable; usage guide has the CSS (`details.tk`) — copy it if needed |
